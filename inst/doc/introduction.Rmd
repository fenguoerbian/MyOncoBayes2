...
hetergeneous sources of data are assigned to *groups* and information
is shared accross groups through a hierarchial model structure. For
...
discounting (or down-weighting) is determined by the heterogeniety. A
group is commonly defined to be a trial, but that must not necessarily
hold.

The key assumption of the hierarhical model is the exchangability
...
strata. While the overal hierarchical mean stays the same, the
heterogeniety between groups is allowed to be different between
...
EXNEX each group is modelled as being exchangable with some
probability and is allowed to have it's own group-specific estimate
as if the grup is not exchangable with the remainder of the data.
...
For simplicity and concicneness purposes, here we use method #1, which
is not recommended for actual trials as the prior should be choosen
deliberatley and there is no gurantee that the simplified prior will
remain stable accross releases of the package. See
...
admissable dose allowed by EWOC towards higher doses for drug B
...
In the example of [1], during the conduct of the second satge of the
...
then the EWOC criterium is violated and the dose is too toxic.
