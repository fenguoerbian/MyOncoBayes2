...
)
{
  ...
  # Add individual compound terms
  for (compound_index in seq(1, num_components))
  {
    blrm_formula <- paste0(
      blrm_formula,
      "1 + I(log(", component_names[compound_index], "/",
      ref_doses[[component_names[compound_index]]], ")) | "
    )
  }
  ...
  if (num_components >= 2 && max_interaction_level >=2) {
    ...
    for (num_interacting_compounds in interaction_orders)
    {
      for (interaction_compund_names in combn(component_names, num_interacting_compounds, simplify=FALSE))
      {
        blrm_formula <- paste0(blrm_formula, "+ I(")

        # Generate terms that are multiplied in the interaction
        i <- 0
        for (interaction_compound_name in interaction_compund_names)
        {
          if (i > 0)
          {
          ...
          blrm_formula <- paste0(blrm_formula,
            interaction_compound_name ,"/", ref_doses[[interaction_compound_name]]
          )
          i <- i + 1
        }
      ...
      num_interaction_terms <- num_interaction_terms + choose(num_components, num_interacting_compounds)
    }
  }
